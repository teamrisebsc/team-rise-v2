...
const handler = async (event) => {
  ...
  try {
    const res = await fetch(
      `${url}/rest/v1/report_snapshots?report_key=eq.convention&select=data,updated_at&limit=1`,
      { headers: { apikey: key, Authorization: `Bearer ${key}` } }
    )
    if (!res.ok) throw new Error(`Supabase ${res.status}: ${await res.text()}`)
    const rows = await res.json()
    if (!rows?.length) {
      return { statusCode: 200, headers: CORS, body: JSON.stringify({ ok: false, error: 'No convention snapshot yet. Run `node push_convention_snapshot.js` from bscpro-scraper.' }) }
    }
