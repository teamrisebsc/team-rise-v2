...
const handler = async (event) => {
  ...
  if (!SUPABASE_URL || !SUPABASE_KEY) {
    return { statusCode: 200, headers: CORS, body: JSON.stringify({ error: 'Supabase env vars not configured.' }) }
  }
  ...
  try {
    ...
    const job  = rows[0]
    return { statusCode: 200, headers: CORS, body: JSON.stringify({ queued: true, job_id: job.id }) }
  } catch (e) {
